...
export const tasks = pgTable(
  "tasks",
  {
    ...
    // Dates
    periodEndDate: timestamp("period_end_date"), // End of reporting period (e.g., Q1 end = March 31)
    ...
    recurringFrequency: recurringFrequencyEnum("recurring_frequency"), // monthly, quarterly, annually, etc.
    recurringDayOfMonth: integer("recurring_day_of_month"), // For "15th of each month" scenarios (1-31)

    // Auto-generation fields (STORY-3.2)
  ...
  (table) => ({
    ...
    serviceIdx: index("idx_task_service").on(table.serviceId),
    autoGeneratedIdx: index("idx_task_auto_generated").on(table.autoGenerated),
    templateIdx: index("idx_task_template").on(table.templateId),
  }),
);
...
// Workflow Templates - Links workflows to task templates for auto-generation (STORY-3.2)
export const workflowTemplates = pgTable(
  "workflow_templates",
  {
    ...
    workflowId: uuid("workflow_id")
      ...
      .notNull(),
    stageId: text("stage_id"), // Optional: specific workflow stage to trigger on
    templateId: text("template_id")
      .references(() => taskTemplates.id, { onDelete: "cascade" })
      .notNull(),
    triggerType: varchar("trigger_type", { length: 50 }).notNull(), // "on_stage_complete" | "on_workflow_start" | "on_workflow_complete"
    createdAt: timestamp("created_at").defaultNow().notNull(),
    updatedAt: timestamp("updated_at")
      .defaultNow()
      .$onUpdate(() => new Date())
  ...
  (table) => ({
    tenantIdIdx: index("workflow_templates_tenant_id_idx").on(table.tenantId),
    workflowIdIdx: index("workflow_templates_workflow_id_idx").on(
      table.workflowId,
    ),
    templateIdIdx: index("workflow_templates_template_id_idx").on(
      table.templateId,
    ),
    triggerTypeIdx: index("workflow_templates_trigger_type_idx").on(
      table.triggerType,
    ),
  }),
);
