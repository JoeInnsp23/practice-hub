...
describe('lib/xero/client.ts', () => {
  ...
  describe('getAuthorizationUrl', () => {
    it('should generate valid OAuth URL with state parameter', () => {
      ...
      expect(url).toContain('redirect_uri=http%3A%2F%2Flocalhost%3A3000%2Fapi%2Fxero%2Fcallback');
  ...
  describe('getAccessToken', () => {
    ...
    it('should throw error on 401 Unauthorized response', async () => {
      ...
      await expect(getAccessToken('invalid-code')).rejects.toThrow('Xero token exchange failed: Unauthorized');
    });
    ...
    it('should throw error on 500 Server Error response', async () => {
      ...
      await expect(getAccessToken('test-code')).rejects.toThrow('Xero token exchange failed: Internal Server Error');
    });
  ...
  describe('refreshAccessToken', () => {
    ...
    it('should throw error when refresh token is expired', async () => {
      ...
      await expect(refreshAccessToken('expired_token')).rejects.toThrow('Xero token refresh failed: Refresh token expired');
    });
  ...
  describe('getConnections', () => {
    ...
    it('should throw error when fetch fails', async () => {
      ...
      await expect(getConnections('invalid-token')).rejects.toThrow('Failed to fetch Xero connections');
    });
  });
  ...
  describe('getValidAccessToken', () => {
    ...
    it('should throw error when no Xero connection found for client', async () => {
      ...
      await expect(getValidAccessToken('non-existent-client')).rejects.toThrow('No Xero connection found for this client');
    });
  });
  ...
  describe('fetchBankTransactions', () => {
    it('should fetch bank transactions successfully', async () => {
      ...
      const result = await fetchBankTransactions('test-token', 'tenant-1', fromDate, toDate);
      ...
      expect(result[1].total).toBe(200.00);
      expect(result[1].type).toBe('RECEIVE');

      expect(global.fetch).toHaveBeenCalledWith(
        expect.stringContaining('https://api.xero.com/api.xro/2.0/BankTransactions'),
        expect.objectContaining({
          headers: expect.objectContaining({
    ...
    it('should handle empty transactions response', async () => {
      ...
      const result = await fetchBankTransactions('test-token', 'tenant-1', new Date(), new Date());
    ...
    it('should throw error on API failure', async () => {
      ...
      await expect(
        fetchBankTransactions('invalid-token', 'tenant-1', new Date(), new Date())
  ...
  describe('Error Handling', () => {
    describe('Network Errors', () => {
      it('should handle network error in getAccessToken', async () => {
        ...
        await expect(getAccessToken('test-code')).rejects.toThrow('Network error');
      });
      ...
      it('should handle network error in refreshAccessToken', async () => {
        ...
        await expect(refreshAccessToken('test-token')).rejects.toThrow('Connection timeout');
      });
      ...
      it('should handle network error in getConnections', async () => {
        ...
        await expect(getConnections('test-token')).rejects.toThrow('DNS resolution failed');
      });
      ...
      it('should handle network error in fetchBankTransactions', async () => {
        ...
        await expect(
          fetchBankTransactions('test-token', 'tenant-1', new Date(), new Date())
    ...
    describe('HTTP Error Codes', () => {
      it('should handle 429 Rate Limit error in getAccessToken', async () => {
        ...
        await expect(getAccessToken('test-code')).rejects.toThrow('Xero token exchange failed: Rate limit exceeded');
      });
      ...
      it('should handle 500 Internal Server Error in refreshAccessToken', async () => {
        ...
        await expect(refreshAccessToken('test-token')).rejects.toThrow('Xero token refresh failed: Internal server error');
      });
    });
    ...
    describe('Malformed Responses', () => {
      it('should handle invalid JSON in getAccessToken', async () => {
        ...
        await expect(getAccessToken('test-code')).rejects.toThrow('Unexpected token in JSON');
      });
  ...
  describe('calculateMonthlyTransactions', () => {
    ...
    it('should handle single month with multiple transactions', () => {
      const transactions: BankTransaction[] = [
        { date: '2025-01-05', type: 'SPEND', total: 100, description: 'Test 1', reference: 'REF1' },
        { date: '2025-01-15', type: 'SPEND', total: 200, description: 'Test 2', reference: 'REF2' },
        { date: '2025-01-25', type: 'RECEIVE', total: 150, description: 'Test 3', reference: 'REF3' },
      ];
    ...
    it('should handle single transaction in single month', () => {
      const transactions: BankTransaction[] = [
        { date: '2025-01-15', type: 'SPEND', total: 100, description: 'Test', reference: 'REF1' },
      ];
    ...
    it('should handle transactions across multiple months', () => {
      const transactions: BankTransaction[] = [
        { date: '2025-01-10', type: 'SPEND', total: 100, description: 'Jan 1', reference: 'REF1' },
        { date: '2025-01-20', type: 'SPEND', total: 200, description: 'Jan 2', reference: 'REF2' },
        { date: '2025-02-05', type: 'RECEIVE', total: 150, description: 'Feb 1', reference: 'REF3' },
        { date: '2025-03-12', type: 'SPEND', total: 75, description: 'Mar 1', reference: 'REF4' },
        { date: '2025-03-22', type: 'RECEIVE', total: 125, description: 'Mar 2', reference: 'REF5' },
        { date: '2025-03-28', type: 'SPEND', total: 90, description: 'Mar 3', reference: 'REF6' },
      ];
    ...
    it('should round fractional averages correctly', () => {
      const transactions: BankTransaction[] = [
        { date: '2025-01-10', type: 'SPEND', total: 100, description: 'Test 1', reference: 'REF1' },
        { date: '2025-01-20', type: 'SPEND', total: 200, description: 'Test 2', reference: 'REF2' },
        { date: '2025-02-05', type: 'RECEIVE', total: 150, description: 'Test 3', reference: 'REF3' },
        { date: '2025-03-12', type: 'SPEND', total: 75, description: 'Test 4', reference: 'REF4' },
        { date: '2025-04-15', type: 'RECEIVE', total: 125, description: 'Test 5', reference: 'REF5' },
      ];
