...
describe.skipIf(!process.env.XERO_TEST)('Xero Integration Tests (Manual Only)', () => {
  ...
  describe('OAuth Authorization Flow', () => {
    ...
    it.skip('should exchange authorization code for token', async () => {
      ...
      console.log('Access token received (first 20 chars):', tokenResponse.access_token.substring(0, 20));
      console.log('Refresh token received (first 20 chars):', tokenResponse.refresh_token.substring(0, 20));
  ...
  describe('Token Refresh', () => {
    it.skip('should refresh access token', async () => {
      ...
      console.log('New access token received (first 20 chars):', tokenResponse.access_token.substring(0, 20));
    });
  });
  ...
  describe('Bank Transactions', () => {
    it.skip('should fetch bank transactions from Xero sandbox', async () => {
      ...
      console.log('Found', transactions.length, 'transaction(s) in last 30 days');
  ...
  describe('End-to-End Integration Test', () => {
    it.skip('should complete full OAuth flow and fetch data', async () => {
      ...
      const tokenResponse = await getAccessToken(authCode);
      console.log('✓ Token exchange successful');
      console.log('Access token (first 20):', tokenResponse.access_token.substring(0, 20));
      console.log('Refresh token (first 20):', tokenResponse.refresh_token.substring(0, 20));
      console.log('Expires in:', tokenResponse.expires_in, 'seconds\n');
...
describe('Xero Integration Tests - Setup Instructions', () => {
  it('should show setup instructions when XERO_TEST is not set', () => {
    if (!process.env.XERO_TEST) {
      ...
      console.log('1. Set up Xero sandbox account (https://developer.xero.com)');
      console.log('2. Add credentials to .env.local');
      console.log('3. Run: XERO_TEST=true pnpm test lib/xero/client.integration.test.ts');
      console.log('========================================\n');
    }
