...
export function TaskNotesSection({ taskId }: TaskNotesSectionProps) {
  ...
  // Queries
  const { data: notes = [], isLoading } = trpc.tasks.getNotes.useQuery({
    taskId,
  });
  ...
  const createNote = trpc.tasks.createNote.useMutation({
    onSuccess: () => {
      ...
      utils.tasks.getNoteCount.invalidate({ taskId });
    },
    onError: (error) => {
  ...
  const updateNote = trpc.tasks.updateNote.useMutation({
    onSuccess: () => {
  ...
  const deleteNote = trpc.tasks.deleteNote.useMutation({
    onSuccess: () => {
