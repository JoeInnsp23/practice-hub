  ...
  kpis: protectedProcedure.query(async ({ ctx }) => {
    try {
      ...
      const kpis = {
        ...
        utilizationRate:
          Number(kpiData.total_hours_30d || 0) > 0
            ? (Number(kpiData.billable_hours_30d) /
                Number(kpiData.total_hours_30d)) *
              100
            : 0,
        collectionRate:
          Number(kpiData.total_revenue || 0) > 0
            ? (Number(kpiData.collected_revenue) /
                Number(kpiData.total_revenue)) *
              100
            : 0,
      };
