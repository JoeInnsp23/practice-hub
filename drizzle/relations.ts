...
import { tenants, activityLogs, users, amlChecks, clients, onboardingSessions, announcements, calendarEvents, tasks, compliance, clientContacts, clientDirectors, clientPscs, clientPortalAccess, clientPortalUsers, account, clientPortalInvitations, clientPortalSession, clientPortalAccount, clientPortalVerification, clientTaskTemplateOverrides, taskTemplates, clientTransactionData, leads, clientServices, services, emailQueue, emailTemplates, feedback, importLogs, departments, documents, invoices, kycVerifications, invitations, leaveBalances, leaveRequests, legalPages, messageThreads, messages, notifications, onboardingResponses, portalCategories, portalLinks, pricingRules, proposalNotes, proposals, proposalSignatures, proposalTemplates, proposalVersions, proposalServices, onboardingTasks, session, staffCapacity, taskAssignmentHistory, taskNotes, workflowVersions, taskWorkflowInstances, workflows, rolePermissions, toilAccrualHistory, timesheetSubmissions, userFavorites, userPermissions, userSettings, workTypes, timeEntries, workflowEmailRules, workflowStages, calendarEventAttendees, documentSignatures, integrationSettings, invoiceItems, messageThreadParticipants, workflowTemplates, workingPatterns, xeroConnections, xeroWebhookEvents } from "./schema";
...
export const clientDirectorsRelations = relations(clientDirectors, ({one}) => ({
	tenant: one(tenants, {
...
export const clientPortalInvitationsRelations = relations(clientPortalInvitations, ({one}) => ({
	tenant: one(tenants, {
...
export const clientPortalSessionRelations = relations(clientPortalSession, ({one}) => ({
	tenant: one(tenants, {
...
export const clientPortalAccountRelations = relations(clientPortalAccount, ({one}) => ({
	tenant: one(tenants, {
...
export const clientPortalVerificationRelations = relations(clientPortalVerification, ({one}) => ({
	tenant: one(tenants, {
...
export const clientTaskTemplateOverridesRelations = relations(clientTaskTemplateOverrides, ({one}) => ({
	tenant: one(tenants, {
...
export const clientTransactionDataRelations = relations(clientTransactionData, ({one}) => ({
	tenant: one(tenants, {
...
export const portalCategoriesRelations = relations(portalCategories, ({one, many}) => ({
	tenant: one(tenants, {
...
export const proposalSignaturesRelations = relations(proposalSignatures, ({one}) => ({
	tenant: one(tenants, {
...
export const taskAssignmentHistoryRelations = relations(taskAssignmentHistory, ({one}) => ({
	tenant: one(tenants, {
...
export const taskWorkflowInstancesRelations = relations(taskWorkflowInstances, ({one}) => ({
	workflowVersion_workflowVersionId: one(workflowVersions, {
		fields: [taskWorkflowInstances.workflowVersionId],
		references: [workflowVersions.id],
		relationName: "taskWorkflowInstances_workflowVersionId_workflowVersions_id"
	}),
	workflowVersion_upgradedFromVersionId: one(workflowVersions, {
		fields: [taskWorkflowInstances.upgradedFromVersionId],
		references: [workflowVersions.id],
		relationName: "taskWorkflowInstances_upgradedFromVersionId_workflowVersions_id"
	}),
	task: one(tasks, {
...
export const toilAccrualHistoryRelations = relations(toilAccrualHistory, ({one}) => ({
	tenant: one(tenants, {
...
export const timesheetSubmissionsRelations = relations(timesheetSubmissions, ({one, many}) => ({
	toilAccrualHistories: many(toilAccrualHistory),
	timeEntries: many(timeEntries),
	tenant: one(tenants, {
...
export const workflowEmailRulesRelations = relations(workflowEmailRules, ({one}) => ({
	tenant: one(tenants, {
...
export const workingPatternsRelations = relations(workingPatterns, ({one}) => ({
	tenant: one(tenants, {
...
export const xeroWebhookEventsRelations = relations(xeroWebhookEvents, ({one}) => ({
	tenant: one(tenants, {
