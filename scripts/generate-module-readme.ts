...
function getModuleKeywords(moduleName: string): string[] {
  const map: Record<string, string[]> = {
    "client-hub": ["client", "contact", "director", "psc", "service", "compliance", "document"],
    "proposal-hub": ["proposal", "lead", "pipeline", "pricing", "onboarding", "template"],
    "practice-hub": ["dashboard", "activity", "notification", "message", "calendar"],
    "admin": ["user", "department", "invitation", "portal", "feedback", "kyc", "staff"],
    "client-portal": ["client_portal", "portal", "onboarding_session", "questionnaire"],
  };
...
function main() {
  ...
  const template = readFileSync("docs/.templates/MODULE_README_TEMPLATE.md", "utf-8");

  let content = template
    .replace(/\[Module Name\]/g, name)
    .replace(/\[path\/to\/module\]/g, modulePath)
    .replace(/\[Hub Module \| Admin Panel \| Client Portal \| Integration Library\]/g, guessModuleType(modulePath))
