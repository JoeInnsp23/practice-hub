...
*This is a placeholder Cookie Policy. It must be reviewed and approved by legal counsel before production deployment.*`,
  ...
  // Create urgent tasks specifically for admin user (for testing urgent tasks widget)
  console.log("Creating urgent tasks for admin user...");
  const today = new Date();
  const urgentTasksData = [
    {
      title: "Review Q4 Tax Returns - Urgent Client Deadline",
      description:
        "Multiple client tax returns need final review before submission deadline. High priority items requiring immediate attention.",
      dueDate: new Date(today.getTime() + 2 * 24 * 60 * 60 * 1000), // Due in 2 days
      priority: "high" as const,
      status: "in_progress" as const,
    },
    {
      title: "Prepare Management Accounts - Board Meeting Tomorrow",
      description:
        "Management accounts needed for board meeting. Client expects comprehensive financial analysis and commentary.",
      dueDate: new Date(today.getTime() + 1 * 24 * 60 * 60 * 1000), // Due tomorrow
      priority: "high" as const,
      status: "pending" as const,
    },
    {
      title: "VAT Return Submission - Deadline Approaching",
      description:
        "VAT return must be submitted within 3 days. All transactions verified and ready for final approval.",
      dueDate: new Date(today.getTime() + 3 * 24 * 60 * 60 * 1000), // Due in 3 days
      priority: "high" as const,
      status: "pending" as const,
    },
    {
      title: "Payroll Processing - Month End",
      description:
        "Monthly payroll processing for multiple clients. Ensure all timesheets approved and calculations verified.",
      dueDate: new Date(today.getTime() + 4 * 24 * 60 * 60 * 1000), // Due in 4 days
      priority: "high" as const,
      status: "in_progress" as const,
    },
    {
      title: "Client Compliance Review - Regulatory Deadline",
      description:
        "Annual compliance review required for regulatory submission. Documentation and supporting evidence must be compiled.",
  ...
  for (const urgentTaskData of urgentTasksData) {
    const urgentClient = faker.helpers.arrayElement(createdClients);
    const task = await db
      .insert(tasks)
      .values({
        tenantId: tenant.id,
        title: urgentTaskData.title,
        description: urgentTaskData.description,
        status: urgentTaskData.status,
        priority: urgentTaskData.priority,
        clientId: urgentClient.id,
        assignedToId: adminUser.id, // Assign to admin user
        createdById: adminUser.id,
        dueDate: urgentTaskData.dueDate,
        targetDate: urgentTaskData.dueDate,
        estimatedHours: String(faker.number.int({ min: 3, max: 8 })),
        progress:
          urgentTaskData.status === "in_progress"
...
<p>Best regards,<br>{company_name}</p>`,
  ...
  // SOP Categories - Industry-standard taxonomy for accountancy practices
  ...
  const topLevelCategories = [
    ...
    },
    {
      ...
      description: "Internal operational processes supporting service delivery",
      sortOrder: 4,
    },
    {
    ...
    },
    {
      ...
      description: "Internal financial control and management procedures",
      sortOrder: 6,
    },
  ];

  const createdTopCategories = await db
