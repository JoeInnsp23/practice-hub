...
test.describe("Invoice Generation", () => {
  test("should create invoice with line items", async ({ page }) => {
    ...
    const createButton = page
      .locator(
        'button:has-text("New Invoice"), button:has-text("Create Invoice"), button:has-text("Add Invoice"), a:has-text("New Invoice")',
      )
      .first();
    ...
    if (await createButton.isVisible({ timeout: 5000 }).catch(() => false)) {
      ...
      const invoiceNumberInput = page
        .locator(
          'input[name*="invoice" i][name*="number" i], input[name="invoiceNumber"], input[placeholder*="invoice" i]',
        )
        .first();
      if (
        await invoiceNumberInput.isVisible({ timeout: 5000 }).catch(() => false)
      ) {
      ...
      const clientSelect = page
        .locator(
          'select[name*="client" i], button:has-text("Select client"), input[name*="client" i]',
        )
        .first();
      if (await clientSelect.isVisible({ timeout: 5000 }).catch(() => false)) {
        // If it's a select/dropdown
        if (
          await clientSelect.evaluate(
            (el) => el.tagName === "SELECT" || el.role === "combobox",
          )
        ) {
      ...
      const dateInput = page
        .locator('input[type="date"][name*="date" i], input[type="date"]')
        .first();
      ...
      const addItemButton = page
        .locator(
          'button:has-text("Add Item"), button:has-text("Add Line"), button:has-text("Add Product")',
        )
        .first();
      if (await addItemButton.isVisible({ timeout: 5000 }).catch(() => false)) {
        ...
        const quantityInput = page
          .locator('input[name*="quantity" i], input[placeholder*="qty" i]')
          .first();
        if (
          await quantityInput.isVisible({ timeout: 5000 }).catch(() => false)
        ) {
        ...
        const priceInput = page
          .locator(
            'input[name*="price" i], input[name*="rate" i], input[name*="amount" i]',
          )
          .first();
        ...
        if (
          await addItemButton.isVisible({ timeout: 5000 }).catch(() => false)
        ) {
          await addItemButton.click();
          await page.waitForTimeout(500);
          ...
          const descInputs = await page
            .locator(
              'input[name*="description" i], textarea[name*="description" i]',
            )
            .all();
          ...
          const priceInputs = await page
            .locator('input[name*="price" i], input[name*="rate" i]')
            .all();
        ...
        console.log(
          "Line item functionality not found - creating basic invoice",
        );
      }
      ...
      const submitButton = page
        .locator(
          'button[type="submit"], button:has-text("Save"), button:has-text("Create Invoice"), button:has-text("Generate")',
        )
        .first();
      if (await submitButton.isVisible({ timeout: 5000 }).catch(() => false)) {
        ...
        const invoiceCreated =
          (await page
            .locator("text=/success|created|saved|generated/i")
            .count()) > 0 ||
        ...
        if (invoiceCreated) {
          ...
          const previewButton = page
            .locator(
              'button:has-text("Preview"), button:has-text("PDF"), button:has-text("View PDF"), a:has-text("PDF")',
            )
            .first();
          if (
            await previewButton.isVisible({ timeout: 5000 }).catch(() => false)
          ) {
  ...
  test("should calculate invoice totals correctly", async ({ page }) => {
    ...
    const createButton = page
      .locator(
        'button:has-text("New Invoice"), button:has-text("Create Invoice"), a:has-text("New Invoice")',
      )
      .first();
    ...
    if (await createButton.isVisible({ timeout: 5000 }).catch(() => false)) {
      ...
      const invoiceNumberInput = page
        .locator(
          'input[name*="invoice" i][name*="number" i], input[name="invoiceNumber"]',
        )
        .first();
      if (
        await invoiceNumberInput.isVisible({ timeout: 5000 }).catch(() => false)
      ...
      if (await addItemButton.isVisible({ timeout: 5000 }).catch(() => false)) {
        ...
        const totalElements = page.locator(
          'text=/300|£300|\\$300/i, *:has-text("300.00")',
        );
        ...
        if (totalVisible) {
          ...
          if (
            ...
            const savedTotalVisible =
              (await page.locator("text=/300|£300|\\$300/i").count()) > 0;
