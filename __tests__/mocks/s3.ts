...
// Mock getSignedUrl
export const mockGetSignedUrl = vi.fn(async (client: any, command: any, options?: any) => {
...
export const mockUploadToS3 = vi.fn(async (buffer: Buffer, key: string, contentType: string) => {
  return `http://localhost:9000/test-bucket/${key}`;
});

// Mock getPresignedUrl function
export const mockGetPresignedUrl = vi.fn(async (key: string, expiresIn: number = 3600) => {
  return `https://mock-s3.example.com/test-bucket/${key}?X-Amz-Expires=${expiresIn}&X-Amz-Signature=mock-signature`;
});
