...
function extractTypeScriptDocs(filePath: string): CodeEntry[] {
  ...
  const jsdocPattern = /\/\*\*\s*([\s\S]*?)\*\/\s*export\s+((?:type|function|const|default function|interface|class)\s+(\w+))/g
  ...
  while ((match = jsdocPattern.exec(content)) !== null) {
    ...
    if (declaration.includes('type ') || declaration.includes('interface ')) type = 'type'
    if (declaration.includes('class ')) type = 'class'
    if (declaration.includes('function') && (filePath.includes('app/') || filePath.includes('components/'))) {
      type = 'component'
    }
...
function main() {
  ...
  const componentFiles = findSourceFiles(path.join(rootDir, 'components'), ['.ts', '.tsx'])
  ...
  console.log(`   - Classes: ${entries.filter(e => e.type === 'class').length}`)
}
